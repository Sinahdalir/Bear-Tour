...
public protocol ARLandmarkerDelegate: class {
    ...
    func updateWorldOriginBeingCalled() -> Void
}
...
extension ARLandmarker {
    ...
    public func updateWorldOrigin(_ origin: CLLocation) {
        delegate?.updateWorldOriginBeingCalled()
        captureDeviceAuthorizer.requestAccess(for: .video) { [weak self] (granted) in
            ...
            DispatchQueue.main.async {
                ...
                landmarksCopy.forEach({ (landmark) in
                    // edit landmark.image to be right size based on origin
